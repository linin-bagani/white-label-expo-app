...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'bg-background-dark-100 dark:bg-background-dark-300 rounded-xl px-5 flex-row justify-between items-center h-[60px] border-gray-300 w-full',
    ...
    <ChevronDown
      size={16}
      aria-hidden={true}
      className="text-foreground dark:text-foreground-light opacity-50"
    />
  </SelectPrimitive.Trigger>
));
...
const SelectScrollUpButton = ({
  ...
  return (
    <SelectPrimitive.ScrollUpButton
      ...
      {...props}>
      <ChevronUp size={14} className="text-foreground dark:text-foreground-light" />
    </SelectPrimitive.ScrollUpButton>
  );
};
      ...
      <SelectPrimitive.Overlay style={Platform.OS !== 'web' ? StyleSheet.absoluteFill : undefined}>
        <Animated.View entering={FadeIn} exiting={FadeOut}>
          <SelectPrimitive.Content
            ref={ref}
            className={cn(
              'relative z-99 max-h-96 min-w-[8rem] rounded-md border border-border bg-popover shadow-md shadow-foreground/10 py-2 px-1 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2',
              position === 'popper' &&
