  ...
  (
    ...
    return (
      <LinearGradient
        ...
        colors={[
          loading || pressed ? '#a855f7' : gradient ? gradient[0] : '#a855f7',
          loading || pressed ? '#a855f7' : gradient ? gradient[0] : '#581c87',
        ]}
        start={[0, 0]}
        end={[1, 0]}>
        <Pressable
          className={buttonClass + (pressed ? 'shadow-none' : 'text-slate-800 pt-6 pb-6')}
          ref={ref}
          onPress={onPress}
          ...
          <Text
            className={textClass}
            // style={{ fontSize: RFValue(16, height) }}
          >
            {children}
          </Text>
        </Pressable>
